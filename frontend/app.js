...
async function cargarGrafoConocimiento() {
  ...
  try {
    container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#94a3b8;font-weight:700;font-size:1.1rem"><i class="fas fa-spinner fa-spin" style="margin-right:10px;font-size:1.5rem"></i> Construyendo Red Graphify...</div>';

    const resAnalytics = await apiCall('/api/analytics');
    const resCarpetas = await apiCall('/api/biblioteca/arbol');

    const nodes = [];
    const edges = [];

    // 1. Nodo Hub Central (Coraza Seguridad C.T.A.)
    nodes.push({
      id: 'HUB_CORAZA',
      label: 'CORAZA SEGURIDAD C.T.A.\n[SISTEMA SGD]',
      shape: 'ellipse',
      color: { background: '#f59e0b', border: '#d97706', highlight: { background: '#fbbf24', border: '#f59e0b' } },
      font: { color: '#ffffff', size: 14, face: 'Syne', strokeWidth: 2, strokeColor: '#000000' },
      typeGroup: 'HUB'
    });

    // 2. Nodos de Departamentos Corporativos (12 Deptos)
    DEPTOS_MOCK.forEach(d => {
      const nodeId = `DEPTO_${d.sigla}`;
      nodes.push({
        id: nodeId,
        label: `${d.nombre}\n(${d.sigla})`,
        shape: 'box',
        color: { background: '#0284c7', border: '#0369a1', highlight: { background: '#38bdf8', border: '#0284c7' } },
        font: { color: '#ffffff', size: 12, face: 'DM Sans' },
        typeGroup: 'DEPARTAMENTO'
      });
      edges.push({ from: 'HUB_CORAZA', to: nodeId, color: { color: 'rgba(2,132,199,0.5)' }, length: 140 });
    });

    // 3. Nodos de Series TRD (Norma AGN) vinculadas a sus departamentos
    Object.keys(MAPA_TRD_DEPTOS).forEach(sigla => {
      const subList = MAPA_TRD_DEPTOS[sigla] || [];
      subList.slice(0, 3).forEach((trdItem, idx) => {
        const trdId = `TRD_${sigla}_${idx}`;
        nodes.push({
          id: trdId,
          label: `TRD ${trdItem.val}\n${trdItem.label.split('/')[1] || ''}`,
          shape: 'dot',
          size: 14,
        ...
        edges.push({ from: `DEPTO_${sigla}`, to: trdId, color: { color: 'rgba(139,92,246,0.4)' }, dashes: true });
      });
    });

    // 4. Nodos de Compartimentos de Archivo Físico Voxelsera (A, B, C, D)
    const voxels = [
      { id: 'VOX_A', name: 'Estante A\nMinutas', color: '#10b981', deptoLink: 'OP' },
      { id: 'VOX_B', name: 'Estante B\nAsociados Retirados', color: '#10b981', deptoLink: 'GH' },
      { id: 'VOX_C', name: 'Estante C\nContratos', color: '#10b981', deptoLink: 'DJ' },
      { id: 'VOX_D', name: 'Estante D\nCorrespondencia', color: '#10b981', deptoLink: 'GE' }
    ];
    voxels.forEach(v => {
      nodes.push({
        id: v.id,
        label: v.name,
        shape: 'database',
        color: { background: v.color, border: '#059669' },
        font: { color: '#ffffff', size: 11, face: 'DM Sans' },
        typeGroup: 'VOXELSERA'
      });
      edges.push({ from: `DEPTO_${v.deptoLink}`, to: v.id, color: { color: 'rgba(16,185,129,0.5)' }, width: 2 });
    });

    // 5. Nodos de Biblioteca Documental & Contratos
    if (resCarpetas && resCarpetas.carpetas) {
      resCarpetas.carpetas.forEach(c => {
        const cId = `BIB_${c.id}`;
        nodes.push({
          id: cId,
          label: `Carpeta: ${c.nombre}`,
          shape: 'diamond',
          size: 12,
        ...
        edges.push({ from: 'DEPTO_GE', to: cId, color: { color: 'rgba(236,72,153,0.4)' } });
      });
    }
    ...
    const badgeNodes = document.getElementById('badgeGrafoNodes');
    if (badgeNodes) badgeNodes.textContent = `${nodes.length} nodos · ${edges.length} conexiones`;

    // Renderizar gráfico interactivo con Vis.js
    if (typeof vis !== 'undefined') {
      ...
      const options = {
        ...
        physics: {
          barnesHut: { gravitationalConstant: -3000, centralGravity: 0.3, springLength: 110 },
      ...
      window.networkGrafo = new vis.Network(container, data, options);

      // Evento al hacer clic en un nodo del Grafo
      window.networkGrafo.on('click', function(params) {
        if (params.nodes.length > 0) {
          const selectedId = params.nodes[0];
          const nodeObj = nodes.find(n => n.id === selectedId);
          if (nodeObj) {
            Swal.fire({
              title: `🌐 Nodo Graphify: ${nodeObj.label.replace('\n', ' ')}`,
              text: `Grupo: ${nodeObj.typeGroup} | ID: ${nodeObj.id}`,
              icon: 'info',
      ...
      container.innerHTML = '<div class="alert alert-warning" style="margin:40px">Librería Vis.js no disponible para renderizar el grafo.</div>';
    }
  } catch(e) {
    container.innerHTML = '<div class="alert alert-danger" style="margin:40px">Error al construir el Grafo de Conocimiento: ' + e.message + '</div>';
  }
}

function filtrarNodosGrafo() {
  if (!window.networkGrafo || !window.allGrafoNodes) return;
  const filtro = document.getElementById('filtroGrafoTipo')?.value || 'TODOS';

  let filteredNodes = window.allGrafoNodes;
  if (filtro !== 'TODOS') {
    filteredNodes = window.allGrafoNodes.filter(n => n.typeGroup === 'HUB' || n.typeGroup === filtro);
  }

  const validIds = new Set(filteredNodes.map(n => n.id));
  const filteredEdges = window.allGrafoEdges.filter(e => validIds.has(e.from) && validIds.has(e.to));
  ...
  window.networkGrafo.setData(data);
}

function estabilizarGrafo() {
  if (window.networkGrafo) {
    window.networkGrafo.stabilize();
  }
}

function reiniciarZoomGrafo() {
  if (window.networkGrafo) {
    window.networkGrafo.fit({ animation: { duration: 800, easingFunction: 'easeInOutQuad' } });
  }
}
